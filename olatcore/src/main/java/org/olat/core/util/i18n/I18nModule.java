...
public class I18nModule extends AbstractOLATModule implements Destroyable {
	...
	private static final Map<String, File> translatableLangCoreBaseDirLookup = new HashMap<String, File>();
	...
	private void doInitAvailableLanguages() {
		...
		if (availableLanguages.size() == 0 || !availableLanguages.contains(Locale.ENGLISH.toString())) { throw new OLATRuntimeException(
				"Did not find any language files, not even 'en'! At least 'en' must be available. NOTE: IN DEVELOPMENT ENVIRONMENTS, try setting olatcore.src in the olat.properties file to something like <your_workspace_dir>/olatcore/src/main/java/", null); }
