	...
	private void init() {
		...
		try {
			...
			if (Settings.isDebuging()) {
				// try source first, then fallback to classloader
				// when the brasato source code is attached, look also in the brasato source folder
				String brasatoSrc = WebappHelper.getCoreSourcePath();
				if (brasatoSrc == null) {
					p.setProperty(RuntimeConstants.RESOURCE_LOADER, "file, classpath");					
				} else {
					p.setProperty(RuntimeConstants.RESOURCE_LOADER, "file, basatofile, classpath");
					// config for file lookup from brasato classpath
					p.setProperty("basatofile.resource.loader.class", "org.apache.velocity.runtime.resource.loader.FileResourceLoader");
					p.setProperty("basatofile.resource.loader.path", brasatoSrc);
					p.setProperty("basatofile.resource.loader.cache", "false");
					p.setProperty("basatofile.resource.loader.modificationCheckInterval", "3");					
				}
