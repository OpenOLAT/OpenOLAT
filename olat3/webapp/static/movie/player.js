var BPlayer = {
	insertPlayer: function (address,domId,width,height,start,duration,provider,streamer,autostart,repeat,controlbar) {
		BPlayer.insertFlashPlayer(address,domId,width,height,start,duration,provider,streamer,autostart,repeat,controlbar);
	},
	
	insertHTML5Player : function (address,domId,width,height,start,duration,provider,streamer) {
		var playerContainer = document.getElementById(domId);
		
		var videoUrl = address
		if(address.indexOf('://') < 0 && provider != "rtmp") {
		...
		playerContainer.innerHTML = "<video src='" + videoUrl + "' height='" + height + "' width='" + width + "' controls='controls'></video>";
	},
	...
	insertFlashPlayer : function (address,domId,width,height,start,duration,provider,streamer,autostart,repeat,controlbar) {
		...
		if(start != undefined) {
			...
			if(startInSec > 0) {
				flashvars += "&start=" + startInSec;
			}
		}
		if(duration != undefined) {
			var durationInSec = BPlayer.convertInSeconds(duration);
			if(durationInSec > 0) {
				flashvars += "&duration=" + durationInSec;
			}
		}
		if(autostart) {
		...
		if (swfobject.hasFlashPlayerVersion("9")) {
     		var fn = function() {
        		var att = { data:playerUrl, width:width, height:height, wmode: 'transparent', allowfullscreen: 'true', allownetworking: 'all', allowscriptaccess: 'always'};
