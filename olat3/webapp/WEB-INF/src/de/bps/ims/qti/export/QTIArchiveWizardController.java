...
public class QTIArchiveWizardController extends BasicController {
	...
	public QTIArchiveWizardController(boolean dummyMode, UserRequest ureq, List nodesTableObjectArrayList, OLATResourceable ores, WindowControl wControl) {
		...
		Translator translator = Util.createPackageTranslator(org.olat.ims.qti.export.QTIArchiveWizardController.class, getTranslator().getLocale(), getTranslator());
				  ...
				  Long repKey = RepositoryManager.getInstance().lookupRepositoryEntryBySoftkey(repositorySoftKey, true).getKey();
					...
					if (isOnyx) {
						if (currentCourseNode.getClass().equals(IQSURVCourseNode.class)) {
							...
							.getBasefile() + File.separator + currentCourseNode.getIdent() + File.separator);
	...
	private String exportOnyx(UserRequest ureq, ICourse course) {
		...
		if (currentCourseNode.getClass().equals(IQSURVCourseNode.class)) {
			...
			String surveyPath = course.getCourseEnvironment().getCourseBaseContainer()
				.getBasefile() + File.separator + currentCourseNode.getIdent() + File.separator;
			filename = onyxExportManager.exportResults(surveyPath, exportDir, currentCourseNode);
		} else {
			String repositorySoftKey = (String) currentCourseNode.getModuleConfiguration().get(
					IQEditController.CONFIG_KEY_REPOSITORY_SOFTKEY);
			Long repKey = RepositoryManager.getInstance().lookupRepositoryEntryBySoftkey(repositorySoftKey, true).getKey();
			QTIResultManager qrm = QTIResultManager.getInstance();
