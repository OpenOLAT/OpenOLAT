 ...
 *
 */
public class OAuthValidationResult implements ValidationResult {
	
	private final boolean valid;
	private final List<ValidationDescription> errors = new ArrayList<>();
	
	private OAuthValidationResult(boolean valid) {
		this.valid = valid;
	}
	
	public static final ValidationResult allOk() {
		return new OAuthValidationResult(true);
	}
	
	public static final ValidationResult error(String i18nKey) {
		OAuthValidationResult result = new OAuthValidationResult(false);
