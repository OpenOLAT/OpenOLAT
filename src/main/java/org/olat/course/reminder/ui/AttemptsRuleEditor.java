 ...
 *
 */
public class AttemptsRuleEditor extends RuleEditorFragment {
	...
	public AttemptsRuleEditor(ReminderRule rule, RepositoryEntry entry) {
		super(rule);
		this.entry = entry;
	...
	public FormItem initForm(FormItemContainer formLayout, Controller listener, UserRequest ureq) {
		...
		for(int i=0; i<attemptableNodes.size(); i++) {
			CourseNode attemptableNode = attemptableNodes.get(1);
