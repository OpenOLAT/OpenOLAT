 ...
 *
 */
public class FOConfigsController extends BasicController {
	...
	public FOConfigsController(UserRequest ureq, WindowControl wControl, FOCourseNode courseNode, ICourse course) {
		super(ureq, wControl);
		
		VelocityContainer mainVC = createVelocityContainer("configs");
		
		configCtrl = new FOConfigController(ureq, getWindowControl(), courseNode);
		listenTo(configCtrl);
		mainVC.put("config", configCtrl.getInitialComponent());
		
		if (!courseNode.hasCustomPreConditions()) {
			String contextHelp = forumModule.isAnonymousPostingWithPseudonymEnabled()
					? null
					: "Communication and Collaboration#_forumkonfig";
			
			CourseGroupManager courseGroupManager = course.getCourseEnvironment().getCourseGroupManager();
			nodeRightCtrl = new NodeRightsController(ureq, wControl, courseGroupManager, FOCourseNode.NODE_RIGHT_TYPES,
					courseNode.getModuleConfiguration(), contextHelp);
