 ...
 *
 */
public class BCCourseNodeConfigsController extends BasicController {
	
	private Controller configCtrl;
	private Controller nodeRightCtrl;

	public BCCourseNodeConfigsController(UserRequest ureq, WindowControl wControl, BreadcrumbPanel stackPanel,
			BCCourseNode courseNode, ICourse course) {
		super(ureq, wControl);
		
		VelocityContainer mainVC = createVelocityContainer("configs");
		
		configCtrl = new BCCourseNodeConfigController(ureq, wControl, stackPanel, courseNode, course);
		listenTo(configCtrl);
		...
		if (!courseNode.hasCustomPreConditions()) {
			...
			nodeRightCtrl = new NodeRightsController(ureq, wControl, courseGroupManager, BCCourseNode.NODE_RIGHT_TYPES,
