...
public class CoachFolderController extends BasicController implements Activateable2 {
	...
	public CoachFolderController(UserRequest ureq, WindowControl wControl, UserCourseEnvironment userCourseEnv) {
		...
		SubscriptionContext subContext = CoachFolderFactory.getSubscriptionContext(courseEntry);
		VFSSecurityCallback secCallback = CoachFolderFactory.getSecurityCallback(userCourseEnv, ureq.getUserSession().getRoles().isGuestOnly(), subContext);
		VFSContainer rootContainer = CoachFolderFactory.getFileContainer(userCourseEnv.getCourseEnvironment());
