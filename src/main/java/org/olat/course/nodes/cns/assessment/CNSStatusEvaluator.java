...
import org.olat.course.nodes.st.assessment.STLearningPathStatusEvaluator;
 ...
 *
 */
public class CNSStatusEvaluator extends STLearningPathStatusEvaluator {

	@Override
	public AssessmentEntryStatus getStatus(AssessmentEvaluation currentEvaluation, List<AssessmentEvaluation> children) {
		if (currentEvaluation.getCompletion() == null || currentEvaluation.getCompletion().doubleValue() < 0.001) {
			return AssessmentEntryStatus.notStarted;
		} else if (currentEvaluation.getCompletion().doubleValue() > 0.999) {
