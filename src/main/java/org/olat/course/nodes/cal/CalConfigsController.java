 ...
 *
 */
public class CalConfigsController extends BasicController {
	
	private Controller configCtrl;
	private Controller nodeRightCtrl;

	public CalConfigsController(UserRequest ureq, WindowControl wControl, ICourse course, CalCourseNode courseNode) {
		super(ureq, wControl);
		
		VelocityContainer mainVC = createVelocityContainer("configs");
		
		configCtrl = new CalConfigController(ureq, wControl, courseNode);
		listenTo(configCtrl);
		...
		if (!courseNode.hasCustomPreConditions()) {
			...
			nodeRightCtrl = new NodeRightsController(ureq, getWindowControl(), courseGroupManager,
					CalCourseNode.NODE_RIGHT_TYPES, courseNode.getModuleConfiguration(), null);
