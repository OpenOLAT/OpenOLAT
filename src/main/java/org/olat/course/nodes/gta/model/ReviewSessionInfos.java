/**
 * <a href="https://www.openolat.org">
 * OpenOLAT - Online Learning and Training</a><br>
 * <p>
 * Licensed under the Apache License, Version 2.0 (the "License"); <br>
 * you may not use this file except in compliance with the License.<br>
 * You may obtain a copy of the License at the
 * <a href="http://www.apache.org/licenses/LICENSE-2.0">Apache homepage</a>
 * <p>
 * Unless required by applicable law or agreed to in writing,<br>
 * software distributed under the License is distributed on an "AS IS" BASIS, <br>
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. <br>
 * See the License for the specific language governing permissions and <br>
 * limitations under the License.
 * <p>
 * Initial code contributed and copyrighted by<br>
 * frentix GmbH, https://www.frentix.com
 * <p>
 */
package org.olat.course.nodes.gta.model;

import org.olat.core.id.Identity;
import org.olat.modules.forms.EvaluationFormSession;

/**
 * 
 * Initial date: 7 juin 2024<br>
 * @author srosse, stephane.rosse@frentix.com, http://www.frentix.com
 *
 */
public class ReviewSessionInfos {
	
	private final EvaluationFormSession session;
	
	private double progress = 0.0d;
	
	private final Identity reviewExecutor;
	private final String assessedFullName;
	private final String executorFullName;
	
	public ReviewSessionInfos(EvaluationFormSession session, String executorFullName, String assessedFullName) {
		this.session = session;
		this.reviewExecutor = session.getParticipation().getExecutor();
		this.assessedFullName = assessedFullName;
		this.executorFullName = executorFullName;
	}

	public EvaluationFormSession getSession() {
		return session;
	}

	public Identity getReviewExecutor() {
		return reviewExecutor;
	}
	
	public Long getReviewExecutorKey() {
		return reviewExecutor.getKey();
	}

	public String getAssessedFullName() {
		return assessedFullName;
	}

	public String getExecutorFullName() {
		return executorFullName;
	}

	public double getProgress() {
		return progress;
	}

	public void setProgress(double progress) {
		this.progress = progress;
	}
}
