...
public class SubmissionTaskRuleSPI extends AbstractDueDateTaskRuleSPI {
	...
	public String getStaticText(ReminderRule rule, RepositoryEntry entry, Locale locale) {
		if (rule instanceof ReminderRuleImpl) {
			...
			Date dueDate = null;
			boolean submit = courseNode.getModuleConfiguration().getBooleanSafe(GTACourseNode.GTASK_SUBMIT);
			if(submit) {
				dueDate = courseNode.getModuleConfiguration().getDateValue(GTACourseNode.GTASK_SUBMIT_DEADLINE);
			}
			...
			return translator.translate("rule.submission." + currentUnit, args);
		}
		return null;
	}
