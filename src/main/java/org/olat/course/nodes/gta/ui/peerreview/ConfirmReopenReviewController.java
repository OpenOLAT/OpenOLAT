 ...
 *
 */
public class ConfirmReopenReviewController extends FormBasicController {
	...
	public ConfirmReopenReviewController(UserRequest ureq, WindowControl wControl,
			TaskReviewAssignment assignment, CourseEnvironment courseEnv, GTACourseNode gtaNode) {
		super(ureq, wControl, "confirm_reopen_review", Util.createPackageTranslator(GTACoachController.class, ureq.getLocale()));
	...
	protected void initForm(FormItemContainer formLayout, Controller listener, UserRequest ureq) {
		if(formLayout instanceof FormLayoutContainer layoutCont) {
			...
			layoutCont.contextPut("msg", translate("confirm.reopen.review", reviewer));
		}
		
		uifactory.addFormSubmitButton("reopen.review", formLayout);
	...
	protected void formOK(UserRequest ureq) {
		assignment = peerReviewManager.reopenAssignment(assignment, gtaNode, getIdentity());
		Identity assessedIdentity = assignment.getTask().getIdentity();
		gtaManager.log("Peer reopen", "reopen review", assignment.getTask(), getIdentity(), assessedIdentity, null, courseEnv, gtaNode, Role.coach);
