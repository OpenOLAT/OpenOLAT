 ...
 *
 */
public class ChooseParticipantsListModel extends DefaultFlexiTableDataModel<ChooseParticipantRow>
implements SortableFlexiTableDataModel<ChooseParticipantRow>  {
	
	private static final ParticipantsCols[] COLS = ParticipantsCols.values();
	
	private final Locale locale;
	
	public ChooseParticipantsListModel(FlexiTableColumnModel columnsModel, Locale locale) {
		super(columnsModel);
		this.locale = locale;
	}
	...
	public void sort(SortKey orderBy) {
		if(orderBy != null) {
			List<ChooseParticipantRow> rows = new SortableFlexiTableModelDelegate<>(orderBy, this, locale).sort();
	...
	public Object getValueAt(int row, int col) {
		ChooseParticipantRow participantRow = getObject(row);
		return getValueAt(participantRow, col);
	}

	@Override
	public Object getValueAt(ChooseParticipantRow row, int col) {
		...
		if(col >= 0 && col < COLS.length) {
			switch(COLS[col]) {
				...
				case compensation: return row.getCompensation();
	...
	public enum ParticipantsCols implements FlexiSortableColumnDef {
		...
		compensation("table.header.compensation"),
		;
