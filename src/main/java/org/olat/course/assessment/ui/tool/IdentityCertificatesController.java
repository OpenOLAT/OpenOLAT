...
public class IdentityCertificatesController extends FormBasicController implements GenericEventListener, FlexiTableComponentDelegate {
	...
	private final Certificate excludedCertificate;
	...
	/**
	 * List certificates generated by a course or a certification program. If the list
	 * is from a course and the user has enough permissions, it can generate a new certificate.
	 * This possibility is not allowed for certification programs.
	 * 
	 * @param ureq The user request
	 * @param wControl The window control
	 * @param courseEntry The course (optional)
	 * @param certificateConfig The course configuration
	 * @param certificationProgram The certification program (optional)
	 * @param assessedIdentity The identity which owned the certificates
	 * @param excludedCertificate Exclude a specific certificate of the list
	 * @param admin The user is an administrator (to generate new certificate)
	 * @param readOnly The view is read only
	 */
	public IdentityCertificatesController(UserRequest ureq, WindowControl wControl,
			...
			CertificationProgram certificationProgram, Identity assessedIdentity, Certificate excludedCertificate,
		...
		this.excludedCertificate = excludedCertificate;
		this.certificationProgram = certificationProgram;
		this.titleID = "o_certificates_" + CodeHelper.getRAMUniqueID();
		
		if(!readOnly && certificationProgram == null) {
	...
	protected void loadModel() {
		...
		if(certificationProgram != null) {
			certificates = certificationProgramService.getCertificates(assessedIdentity, certificationProgram);
		} else {
			certificates = certificatesManager.getCertificates(assessedIdentity, courseEntry.getOlatResource());
		}
	...
	public void loadModel(List<Certificate> certificates) {
		...
		for(Certificate certificate:certificates) {
			if(certificate.equals(excludedCertificate)) {
				continue;
			}
