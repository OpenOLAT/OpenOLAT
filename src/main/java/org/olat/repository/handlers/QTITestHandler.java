 ...
 * @author Mike Stock
 * 
 * Comment:  legacy handler for QTI 1.2 test. Only allow to download the content
 * 
 */
	...
	public MainLayoutController createLaunchController(RepositoryEntry re, RepositoryEntrySecurity reSecurity, UserRequest ureq, WindowControl wControl) {
		return new RepositoryEntryRuntimeController(ureq, wControl, re, reSecurity,
			(uureq, wwControl, toolbarPanel, entry, security, assessmentMode) -> {
				...
				return MessageUIFactory.createInfoMessage(uureq, wwControl, "", trans.translate("error.qti12"));
		});
	}
