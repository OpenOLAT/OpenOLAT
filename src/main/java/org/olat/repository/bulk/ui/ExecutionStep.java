...
public class ExecutionStep extends BasicStep {
	...
	public ExecutionStep(UserRequest ureq, SettingsSteps step) {
		super(ureq);
		setTranslator(Util.createPackageTranslator(RepositoryService.class, getLocale(), getTranslator()));
		this.steps = step;
		
		setI18nTitleAndDescr("settings.bulk.execution.title", null);
		setNextStep(RepositoryBulkUIFactory.getNextSettingsStep(ureq, steps, Step.execution));
	}
	...
	public StepFormController getStepController(UserRequest ureq, WindowControl windowControl,
		...
		return new ExecutionController(ureq, windowControl, form, stepsRunContext);
	}
