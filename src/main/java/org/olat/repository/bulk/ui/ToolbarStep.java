 ...
 *
 */
public class ToolbarStep extends BasicStep {

	private final SettingsSteps steps;

	public ToolbarStep(UserRequest ureq, SettingsSteps step) {
		super(ureq);
		setTranslator(Util.createPackageTranslator(RepositoryService.class, getLocale(), getTranslator()));
		this.steps = step;
		
		setI18nTitleAndDescr("settings.bulk.toolbar.title", null);
		setNextStep(RepositoryBulkUIFactory.getNextSettingsStep(ureq, steps, Step.toolbar));
	}
	...
	public StepFormController getStepController(UserRequest ureq, WindowControl windowControl,
		...
		return new ToolbarController(ureq, windowControl, form, stepsRunContext);
	}
