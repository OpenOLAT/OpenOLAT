 ...
 *
 */
public class TaxonomyStep extends BasicStep {

	private final SettingsSteps steps;

	public TaxonomyStep(UserRequest ureq, SettingsSteps step) {
		super(ureq);
		setTranslator(Util.createPackageTranslator(RepositoryService.class, getLocale(), getTranslator()));
		this.steps = step;
		
		setI18nTitleAndDescr("settings.bulk.taxonomy.title", null);
		setNextStep(RepositoryBulkUIFactory.getNextSettingsStep(ureq, steps, Step.taxonomy));
	}
	...
	public StepFormController getStepController(UserRequest ureq, WindowControl windowControl,
		...
		return new TaxonomyController(ureq, windowControl, form, stepsRunContext);
	}
