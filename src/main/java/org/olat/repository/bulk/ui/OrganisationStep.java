 ...
 *
 */
public class OrganisationStep extends BasicStep {

	private final SettingsSteps steps;

	public OrganisationStep(UserRequest ureq, SettingsSteps step) {
		super(ureq);
		setTranslator(Util.createPackageTranslator(RepositoryService.class, getLocale(), getTranslator()));
		this.steps = step;
		
		setI18nTitleAndDescr("settings.bulk.organisation.title", null);
		setNextStep(RepositoryBulkUIFactory.getNextSettingsStep(ureq, steps, Step.organisation));
	}
	...
	public StepFormController getStepController(UserRequest ureq, WindowControl windowControl,
		...
		return new OrganisationController(ureq, windowControl, form, stepsRunContext);
	}
