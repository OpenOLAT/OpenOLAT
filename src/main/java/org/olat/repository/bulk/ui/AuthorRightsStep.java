 ...
 *
 */
public class AuthorRightsStep extends BasicStep {

	private final SettingsSteps steps;

	public AuthorRightsStep(UserRequest ureq, SettingsSteps step) {
		super(ureq);
		setTranslator(Util.createPackageTranslator(RepositoryService.class, getLocale(), getTranslator()));
		this.steps = step;
		
		setI18nTitleAndDescr("settings.bulk.author.rights.title", null);
		setNextStep(RepositoryBulkUIFactory.getNextSettingsStep(ureq, steps, Step.authorRights));
	}
	...
	public StepFormController getStepController(UserRequest ureq, WindowControl windowControl,
		...
		return new AuthorRightsController(ureq, windowControl, form, stepsRunContext);
	}
