...
/**
 * Description: This workflow is used to search for repository entries. The
 * workflow has two steps: 1) a search form and 2) the results list. After
 * calling the constructor nothing happens. The views must be initialized
 * manually. Using some public methods the desired view can be triggered. The
 * workflow can be limited to a specific repository type. Onother option is to
 * set the set the enableSearchforAllReferencalbeInSearchForm In this case, the
 * search workflow can be used to find repository entires that can be referenced
 * by the user. Some doSearch... methods allow the presentation of lists without
 * using the search form at all.
 *
 * @author Felix Jost
 */
public class RepositorySearchController extends BasicController implements Activateable2 {
	...
	public RepositorySearchController(String selectButtonLabel, UserRequest ureq, WindowControl myWControl,
		...
		tableCtr = new TableController(tableConfig, ureq, getWindowControl(), null, null, filterTitle, noFilterOption,
				true, getTranslator());
		if (multiSelect) {
		...
		ColumnDescriptor sortCol = repoTableModel.addColumnDescriptors(tableCtr, selectButtonLabel != null,
				selectButtonLabel != null, false, true, isLifecycleDefVisible(limitTypes));
	...
	/**
	 * @param enableBack true: back link is shown, back goes to search form; false;
	 *                   no back link
	 */
	 ...
	 *               that are referencable/copyable by the current user; false:
	 *               searches done by the search form will find all resources that
	 *               have at least BAR setting in the BARG configuration list
	 */
	...
	/**
	 * Implementation normal search: find repo entries that are public using the
	...
	/**
	 * Implementation of referencable search: find repo entries that are owned by
	 * the uer or set to referencable and have at lease BA settings
	 * 
	 * @param ureq
	 */
	...
	/**
	 * Do search for all resources that the user can reference either because he is
	 * the owner of the resource or because he has author rights and the resource is
	 * set to at least BA in the BARG settings and the resource has the flag
	...
	/**
	 * Do search for all resources that the user can copy either because he is the
	 * owner of the resource or because he has author rights and the resource is set
	 * to at least BA in the BARG settings and the resource has the flag 'canCopy'
	 * set to true.
	 * 
	 * @param owner      The current identity
	 * @param limitTypes List of Types to limit the search
	...
	protected void updateFilters(List<RepositoryEntry> entries, Identity owner) {
		...
		for (RepositoryEntry entry : entries) {
			if (entry.getOlatResource() == null)
				continue;// no red screen for that
		...
		if (source == tableCtr) { // process table actions
			if (event instanceof TableEvent) {
				...
				if (searchType == SearchType.byOwner) {
					doSearchByOwnerLimitTypeInternal(typeFilter.getOwner(), new String[] { typeFilter.getType() },
							false);
