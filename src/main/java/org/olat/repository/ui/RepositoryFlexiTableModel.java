...
public class RepositoryFlexiTableModel extends DefaultFlexiTableDataModel<RepositoryEntry>
	...
	public Object getValueAt(RepositoryEntry re, int col) {
		switch (COLS[col]) {
		case ac:
			return getAccessControl(re);
		case repoEntry:
			return re;
		case displayname:
			return re.getDisplayname();
		case author:
			return getFullname(re.getInitialAuthor());
		case access:
			return re;
		case creationDate:
			return re.getCreationDate();
		case lastUsage:
			return re.getStatistics().getLastUsage();
		case externalId:
			return re.getExternalId();
		case externalRef:
			return re.getExternalRef();
		case lifecycleLabel: {
		...
		case lifecycleStart:
			return re.getLifecycle() == null ? null : re.getLifecycle().getValidFrom();
		case lifecycleEnd:
			return re.getLifecycle() == null ? null : re.getLifecycle().getValidTo();
		default:
			return "ERROR";
		}
	}
