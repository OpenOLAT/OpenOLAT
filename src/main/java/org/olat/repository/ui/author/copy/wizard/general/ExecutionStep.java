 ...
 */
public class ExecutionStep extends BasicStep {

	public static Step create(UserRequest ureq, BasicStepCollection stepCollection, CopyCourseSteps steps) {
		if (steps.isEditExecution()) {
			return new ExecutionStep(ureq, stepCollection, steps);
		} else {
			return CatalogStep.create(ureq, stepCollection, steps);
		}
	}
	
	public ExecutionStep(UserRequest ureq, BasicStepCollection stepCollection, CopyCourseSteps steps) {
		super(ureq);
		
		setTranslator(Util.createPackageTranslator(CopyCourseStepsStep.class, getLocale(), getTranslator()));
		setI18nTitleAndDescr("steps.execution.title", null);
	...
	public StepFormController getStepController(UserRequest ureq, WindowControl windowControl, StepsRunContext stepsRunContext, Form form) {
		return new ExecutionStepController(ureq, windowControl, form, stepsRunContext);
	}
	
	private class ExecutionStepController extends StepFormBasicController {
		
		private RepositoryEntryLifecycleController lifecycleController;
		private CopyCourseContext context;

		public ExecutionStepController(UserRequest ureq, WindowControl wControl, Form rootForm, StepsRunContext runContext) {
