 ...
 */
public class BlogStep extends BasicStep {

	public static Step create(UserRequest ureq, CopyCourseSteps steps) {
		if (steps.isEditBlogSettings()) {
			return new BlogStep(ureq, steps);
		} else {
			return FolderStep.create(ureq, null, steps);
		}
	}

	public BlogStep(UserRequest ureq, CopyCourseSteps steps) {
		super(ureq);

		// Translator and title
		setTranslator(Util.createPackageTranslator(CopyCourseStepsStep.class, getLocale(), getTranslator()));
		setI18nTitleAndDescr("steps.blog", null);
		...
		// Next step
		setNextStep(FolderStep.create(ureq, stepCollection, steps));
	}
	...
	public StepFormController getStepController(UserRequest ureq, WindowControl windowControl, StepsRunContext stepsRunContext, Form form) {
		return new BlogStepController(ureq, windowControl, form, stepsRunContext);
	}

	private class BlogStepController extends StepFormBasicController {

		private CopyCourseContext context;

		private SingleSelection blogCopyModeEl;

		public BlogStepController(UserRequest ureq, WindowControl wControl, Form rootForm, StepsRunContext runContext) {
		...
		protected boolean validateFormLogic(UserRequest ureq) {
			...
			blogCopyModeEl.clearError();

			if (!blogCopyModeEl.isOneSelected()) {
				allOk &= false;
				blogCopyModeEl.setErrorKey("error.select", null);
			}
		...
		protected void formOK(UserRequest ureq) {
			context.setCustomBlogCopyType(CopyType.valueOf(blogCopyModeEl.getSelectedKey()));
