 ...
 *
 */
public class DailyAbsenceRollCallsController extends BasicController {
	
	private final VelocityContainer mainVC;
	private final LecturesAbsenceRollCallsController rollCallsCtrl;
	private final LectureBlockRollCallSearchParameters searchParams;
	
	public DailyAbsenceRollCallsController(UserRequest ureq, WindowControl wControl, Date date, LecturesSecurityCallback secCallback) {
		super(ureq, wControl);
		...
		rollCallsCtrl = new LecturesAbsenceRollCallsController(ureq, wControl, searchParams, true, secCallback);
		listenTo(rollCallsCtrl);
		
		setCurrentDate(date);// load the model
		
		mainVC = createVelocityContainer("daily_absences");
		mainVC.put("noticesList", rollCallsCtrl.getInitialComponent());
