...
implements SortableFlexiTableDataModel<UserRow> {
	...
	public Object getValueAt(UserRow row, int col) {
		if(col >= 0 && col < COLS.length) {
			return switch(COLS[col]) {
				case modifications -> getModifications(row.getMembershipStatus());
				case currentMembership -> row.getMembershipStatus();
				case newMembership -> getNewMembershipStatus(row.getMembershipStatus());
				case organisations -> row.getOrganisations();
				default -> "ERROR";
			};
		}
		
		int propPos = col - UsersOverviewController.USER_PROPS_OFFSET;
		return row.getIdentityProp(propPos);
	}
	
	private ModificationStatus getModifications(UserMembershipStatus currentStatus) {
		if(currentStatus == UserMembershipStatus.ACTIVE) {
			return null;
		}
		if(currentStatus == UserMembershipStatus.ALUMNI) {
			return ModificationStatus.MODIFICATION;
		}
		return ModificationStatus.ADD;
	}
	
	private UserMembershipStatus getNewMembershipStatus(UserMembershipStatus currentStatus) {
		if(currentStatus == UserMembershipStatus.ACTIVE) {
			return null;
		}
	...
	public enum UserOverviewCols implements FlexiSortableColumnDef {
		...
		currentMembership("table.header.current.membership"),
		newMembership("table.header.new.membership"),
