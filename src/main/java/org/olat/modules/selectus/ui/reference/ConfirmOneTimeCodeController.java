 ...
 *
 */
public class ConfirmOneTimeCodeController extends FormBasicController {
	...
	public ConfirmOneTimeCodeController(UserRequest ureq, WindowControl wControl, Reference reference) {
		super(ureq, wControl, "onetimecode", Util.createPackageTranslator(OLATAuthenticationController.class, ureq.getLocale()));
		this.reference = reference;
		this.otp = doSendOneTimeCode();
		initForm(ureq);
	}
	...
	private String doSendOneTimeCode() {
		String code = StringHelper.containsNonWhitespace(otp)
				? otp
				: PasswordGenerator.generateNumericalCode(8);
		try {
			...
			bundle.setTo(reference.getEmail());
