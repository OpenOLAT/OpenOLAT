 ...
 *
 */
public class MemberEditController extends FormBasicController {
	...
	private Map<UserPropertyHandler,TextElement> userPropElements = new HashMap<>();
	...
	private final MembersListDataModel memberList;
	...
	public MemberEditController(UserRequest ureq, WindowControl wControl,  CommitteeMember member, MembersListDataModel memberList, String[] availableRoles) {
		super(ureq, wControl, UserManager.getInstance().getPropertyHandlerTranslator(Util.createPackageTranslator(RecruitingMainController.class, ureq.getLocale(),
				Util.createPackageTranslator(UserCreateController.class, ureq.getLocale()))));
		this.member = member;
		this.memberList = memberList;
		roleKeys = availableRoles;
	...
	protected void initForm(FormItemContainer formLayout, Controller listener, UserRequest ureq) {
		...
		userPropertyHandlers = userManager.getUserPropertyHandlersFor(MembersController.formIdentifyer, true);
		// Add all available user fields to this form
		boolean focused = false;
		for (UserPropertyHandler userPropertyHandler : userPropertyHandlers) {
			if (userPropertyHandler == null) continue;
			FormItem formItem = userPropertyHandler.addFormItem(getLocale(), user, MembersController.formIdentifyer, true, formLayout);
			...
			} else if(formItem instanceof TextElement) {
				userPropElements.put(userPropertyHandler, (TextElement)formItem);
			}
		}
	...
	protected boolean validateFormLogic(UserRequest ureq) {
		...
		if(member.getStatus() != CommitteeMemberStatus.skipped) {
			...
			if (usernameTextElement.isEmpty() || !validationResult.isValid()) {			
				...
				for(CommitteeMember otherMember:memberList.getObjects()) {
					if(otherMember != member && otherMember.getIdentity().getName() != null
							&& otherMember.getIdentity().getName().equals(loginName)) {
						usernameTextElement.setErrorKey("new.error.loginname.choosen");
						allOk &=  false;
					}
				}
	...
	protected void formOK(UserRequest ureq) {
		...
		if(member.getIdentity() == null) {
			member.setIdentity(new TransientIdentity());
		}
		if(member.getIdentity() instanceof TransientIdentity) {
			((TransientIdentity)member.getIdentity()).setName(username);
	...
	private void doSelect(UserRequest ureq, Identity identity) {
		...
		if(identity instanceof IdentityImpl && identity.getKey() != null) {
			...
			for(Map.Entry<UserPropertyHandler, TextElement> entry:userPropElements.entrySet()) {
				String propName = entry.getKey().getName();
				String value = identity.getUser().getProperty(propName, getLocale());
