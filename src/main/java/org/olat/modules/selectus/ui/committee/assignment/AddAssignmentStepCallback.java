 ...
 *
 */
public class AddAssignmentStepCallback implements StepRunnerCallback {
	...
	public AddAssignmentStepCallback(List<ApplicationLight> applications, AssignmentsData data, Translator translator) {
	...
	public Step execute(UserRequest ureq, WindowControl wControl, StepsRunContext runContext) {
		...
		if(data.getAssignmentMethod() == AssignmentMethods.manual) {
			// all applications to all assignees
			assignmentService.assignments(data.getPosition(), applications, assignees, ureq.getIdentity(), translator);
		} else {
			assignmentService.assignments(data.getPosition(), applications, assignees,
					data.getAdditionalAssignments(), data.getMaximumAssignments(), data.getSpreading(),
					ureq.getIdentity(), translator);
		}
		dbInstance.commit();
