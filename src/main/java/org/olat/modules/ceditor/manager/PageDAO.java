...
public class PageDAO {
	...
	public Page linkPageBody(Page newPage, Page existingPage) {
		...
		if(deleteBody) {
			...
			List<PagePart> pageParts = getParts(oldBody);
	...
	public List<Page> getDeletedPages(IdentityRef owner, String searchString) {
		...
		sb.append("select page from cepage as page")
		  ...
		  .append(" where membership.identity.key=:ownerKey and membership.role='").append(ContentRoles.owner.name()).append("'")
	...
	public List<Page> getLastPages(IdentityRef owner, int maxResults) {
		...
		sc.append("select page.key, page.lastModified, body.lastModified, part.lastModified")
		  ...
		  .append("   where pIdent.key=:ownerKey and pageMember.role='").append(ContentRoles.owner.name()).append("'")
		  .append(" )")
		  ...
		  .append("   where bIdent.key=:ownerKey and binderMember.role='").append(ContentRoles.owner.name()).append("'")
		  .append(" )");
	...
	public PageBody persistPart(PageBody body, PagePart part) {
	...
	public PageBody persistPart(PageBody body, PagePart part, int index) {
		AbstractPart aPart = (AbstractPart)part;
		aPart.setCreationDate(new Date());
		aPart.setLastModified(aPart.getCreationDate());
		aPart.setBody(body);
	...
	public int deletePage(Page page) {
		...
		if(deleteBody) {
			...
			List<PagePart> pageParts = getParts(body);
			for (PagePart pagePart : pageParts) {
				pagePart.beforeDelete();
			}
		}
	...
	private void deleteSurvey(PageBody body) {
		EvaluationFormSurvey survey = evaluationFormManager.loadSurvey(PageService.getSurveyIdent(body));
		if (survey != null) {
