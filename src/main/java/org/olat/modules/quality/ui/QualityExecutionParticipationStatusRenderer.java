 ...
 *
 */
public class QualityExecutionParticipationStatusRenderer extends LabelCellRenderer {

	@Override
	protected String getElementCssClass(Object val) {
		if (val instanceof QualityExecutorParticipationStatus) {
			QualityExecutorParticipationStatus status = (QualityExecutorParticipationStatus) val;
			return "o_qual_exec_status_" + status.name().toLowerCase() + "_light";
		}
		return null;
	}
	...
	protected String getIconCssClass(Object val) {
		if (val instanceof QualityExecutorParticipationStatus) {
			QualityExecutorParticipationStatus status = (QualityExecutorParticipationStatus) val;
			return "o_icon_qual_exec_" + status.name().toLowerCase();
		}
		return null;
	}
