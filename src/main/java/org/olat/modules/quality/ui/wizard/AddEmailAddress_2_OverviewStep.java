 ...
 *
 */
public class AddEmailAddress_2_OverviewStep extends BasicStep {
	
	public AddEmailAddress_2_OverviewStep(UserRequest ureq) {
		super(ureq);
		setNextStep(Step.NOSTEP);
		setTranslator(Util.createPackageTranslator(ParticipationListController.class, getLocale(), getTranslator()));
		setI18nTitleAndDescr("participation.user.email.overview", "participation.user.email.overview");
	}
	...
	public StepFormController getStepController(UserRequest ureq, WindowControl wControl, StepsRunContext runContext, Form form) {
		return new AddEmailOverviewController(ureq, wControl, form, runContext);
	}
}