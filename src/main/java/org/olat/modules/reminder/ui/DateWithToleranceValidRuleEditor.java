 ...
 *
 */
public class DateWithToleranceValidRuleEditor extends RuleEditorFragment {
	...
	private TextElement toleranceEl;
	private MultipleSelectionElement enableToleranceEl;
	
	private final String ruleType;
	
	public DateWithToleranceValidRuleEditor(ReminderRule rule, String ruleType) {
		super(rule);
	...
	public FormItem initForm(FormItemContainer formLayout, Controller listener, UserRequest ureq) {
		...
		String page = Util.getPackageVelocityRoot(this.getClass()) + "/date_with_tolerance.html";
		...
		String toleranceInDay = StringHelper.isLong(currentTolerance) ? currentTolerance : null;
		SelectionValues enableTolerancePK = new SelectionValues();
		enableTolerancePK.add(SelectionValues.entry("on", trans.translate("ignore.overdue")));
		enableToleranceEl = uifactory.addCheckboxesHorizontal("enabletolerance".concat(id), null, ruleCont,
				enableTolerancePK.keys(), enableTolerancePK.values());
		enableToleranceEl.addActionListener(FormEvent.ONCHANGE);
		enableToleranceEl.setDomReplacementWrapperRequired(false);
		enableToleranceEl.getComponent().setSpanAsDomReplaceable(true);
		if(StringHelper.containsNonWhitespace(toleranceInDay)) {
			enableToleranceEl.select("on", true);
		}
		
		toleranceEl = uifactory.addTextElement("tolerance".concat(id), null, 4, toleranceInDay, ruleCont);
		toleranceEl.setVisible(enableToleranceEl.isAtLeastSelected(1));
		toleranceEl.setDomReplacementWrapperRequired(false);
		toleranceEl.setTextAddOn("tolerance.addon");
		toleranceEl.setDisplaySize(3);
		return ruleCont;
	}
	
	@Override
	public void formInnerEvent(UserRequest ureq, FormItem source, FormEvent event) {
		if(enableToleranceEl == source) {
			toleranceEl.setVisible(enableToleranceEl.isAtLeastSelected(1));
		}
	}
	...
	public ReminderRule getConfiguration() {
		...
		if(enableToleranceEl.isAtLeastSelected(1) && StringHelper.isLong(toleranceEl.getValue())) {
			configuredRule.setTolerance(toleranceEl.getValue());
			configuredRule.setToleranceUnit(LaunchUnit.day.name());
		}
