...
public class RoleProjectSecurityCallback implements ProjProjectSecurityCallback {
	...
	@Override
	public boolean canDeleteFilePermanently(ProjFile file, IdentityRef identity) {
		return canDeleteArtefactPermanently(file.getArtefact(), identity);
	}
	...
	private boolean canDeleteArtefactPermanently(ProjArtefact artefact, IdentityRef identity) {
		return !projectReadOnly 
				&& ProjectStatus.deleted == artefact.getStatus()
				&& (templateManager || hasRole(PROJECT_ADMIN) || identity.getKey().equals(artefact.getCreator().getKey()));
	}
