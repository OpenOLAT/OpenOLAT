...
public class ImageMapper implements Mapper {
	...
	private VFSLeaf backgroundImage;

	private BindersDataModel binderModel;
	...
	public ImageMapper(VFSLeaf backgroundImage) {
	...
	public MediaResource handle(String relPath, HttpServletRequest request) {
		...
		// if binderModel is not null, that means the call is coming from (last)binderListCtrl
		if (binderModel != null) {
			...
			// else the call is from courseNode Portfolio
			VFSLeaf image = backgroundImage;
			VFSLeaf thumbnail = vfsRepositoryService.getThumbnail(image, BACKGROUND_SIZE.getWidth(), BACKGROUND_SIZE.getHeight(), true);
			if (thumbnail != null) {
				image = thumbnail;
			}
