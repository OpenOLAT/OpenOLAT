...
public class SharedWithMeQueries {
	...
	public List<AssessedPage> searchSharedPagesEntries(Identity member, SearchSharePagesParameters params) {
		...
		sb.append("select binder.key,")
		  ...
		  .append("  (select max(part.lastModified) from cepagepart as part")
		  ...
		  .append(params.isBookmarkOnly() ? " inner" : " left").append(" join cepageuserinfos as uinfos on (uinfos.page.key=page.key and uinfos.identity.key=:identityKey)")
		  .append(" where ");
		...
		sb.append(" (exists (select membership.key from bgroupmember as membership")
		  ...
		  .append(" ) or exists (select page.key from cepage as coachedPage")
