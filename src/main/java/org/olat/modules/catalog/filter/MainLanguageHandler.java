...
public class MainLanguageHandler implements CatalogFilterHandler {
	...
	public FlexiTableExtendedFilter createFlexiTableFilter(Translator translator, CatalogFilter catalogFilter,
		...
		catalogEntries.stream()
				.map(CatalogEntry::getMainLanguage)
				.filter(StringHelper::containsNonWhitespace)
				.map(StringHelper::escapeHtml)
				.distinct()
				.sorted()
				.forEach(mainLanguage -> filterSV.add(new SelectionValue(
						mainLanguage,
						mainLanguage)));
		...
		return new FlexiTableMultiSelectionFilter(translator.translate("cif.mainLanguage"), TYPE, filterSV,
	...
	public void filter(FlexiTableFilter flexiTableFilter, List<CatalogEntryRow> rows) {
		...
		if (mainLanguages != null && !mainLanguages.isEmpty()) {
			rows.removeIf(row -> row.getMainLanguage() == null || !mainLanguages.contains(row.getMainLanguage()));
		}
	}
