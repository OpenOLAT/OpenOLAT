/**
 * <a href="http://www.openolat.org">
 * OpenOLAT - Online Learning and Training</a><br>
 * <p>
 * Licensed under the Apache License, Version 2.0 (the "License"); <br>
 * you may not use this file except in compliance with the License.<br>
 * You may obtain a copy of the License at the
 * <a href="http://www.apache.org/licenses/LICENSE-2.0">Apache homepage</a>
 * <p>
 * Unless required by applicable law or agreed to in writing,<br>
 * software distributed under the License is distributed on an "AS IS" BASIS, <br>
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. <br>
 * See the License for the specific language governing permissions and <br>
 * limitations under the License.
 * <p>
 * Initial code contributed and copyrighted by<br>
 * frentix GmbH, http://www.frentix.com
 * <p>
 */
package org.olat.modules.opencast.manager.client;

import java.util.HashMap;
import java.util.Map;
import java.util.stream.Collectors;

import org.olat.modules.opencast.AuthDelegate;

/**
 * 
 * Initial date: 10 Aug 2020<br>
 * @author uhensler, urs.hensler@frentix.com, http://www.frentix.com
 *
 */
public class GetSeriesParams {
	
	public enum Filter {
		title,
		textFilter
	}
	public enum Sorter {
		title
	}
	
	private AuthDelegate authDelegate;
	private String limit;
	private String filterParam;
	private String sortParam;
	
	private GetSeriesParams(Builder builder) {
		this.authDelegate = builder.authDelegate;
		
		if (builder.limit != null) {
			this.limit = Integer.toString(builder.limit);
		}
		
		if (builder.filters != null) {
			filterParam = builder.filters.entrySet().stream()
					.map(entry -> entry.getKey().name() + ":" + entry.getValue())
					.collect(Collectors.joining(","));
		}
		
		if (builder.sorters != null) {
			sortParam = builder.sorters.entrySet().stream()
					.map(entry -> entry.getKey().name() + ":" + (entry.getValue().booleanValue()? "ASC": "DESC"))
					.collect(Collectors.joining(","));
		}
	}

	public AuthDelegate getAuthDelegate() {
		return authDelegate;
	}

	public String getLimit() {
		return limit;
	}

	public String getFilterParam() {
		return filterParam;
	}
	
	public String getSortParam() {
		return sortParam;
	}
	
	public static Builder builder() {
		return new Builder();
	}

	public static final class Builder {
		
		private AuthDelegate authDelegate;
		private Integer limit;
		private Map<Filter, String> filters;
		private Map<Sorter, Boolean> sorters;
		
		public Builder setAuthDelegate(AuthDelegate authDelegate) {
			this.authDelegate = authDelegate;
			return this;
		}
		
		public Builder setLimit(int limit) {
			this.limit = Integer.valueOf(limit);
			return this;
		}
		
		public Builder addFilter(Filter filter, String value) {
			if (filters == null) {
				filters = new HashMap<>();
			}
			filters.put(filter, value);
			return this;
		}
		
		public Builder addSorter(Sorter sorter, boolean asc) {
			if (sorters == null) {
				sorters = new HashMap<>();
			}
			sorters.put(sorter, Boolean.valueOf(asc));
			return this;
		}
		
		public GetSeriesParams build() {
			return new GetSeriesParams(this);
		}
	}

}
