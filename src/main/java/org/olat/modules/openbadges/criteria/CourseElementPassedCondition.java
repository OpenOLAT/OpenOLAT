...
public class CourseElementPassedCondition implements BadgeCondition {
	...
	public String toString(Translator translator, RepositoryEntry courseEntry) {
		...
		searchParams.setSubIdents(Collections.singletonList(getSubIdent()));
		List<CourseElement> courseElements = courseElementDAO.load(searchParams);
		if (courseElements.isEmpty()) {
			return translator.translate("badgeCondition." + KEY, courseEntry.getKey() + ", " + getSubIdent());
		}

		return translator.translate("badgeCondition." + KEY, courseElements.get(0).getShortTitle());
	}
