...
public class MaxPrioritiesCriterion implements TBEnrollmentStrategyCriterion {
	...
	public MaxPrioritiesCriterion(int maxSelections) {
	...
	public double getValue(List<TBSelection> selections) {
		long sumHighestPriorities = 0;
		long sumPriorities = 0;
		
		for (TBSelection selection : selections) {
			if (selection.isEnrolled()) {
				sumHighestPriorities += maxSelections;
				// linear
				sumPriorities += (maxSelections - selection.getSortOrder() + 1);
			}
		}
		
		return sumHighestPriorities > 0? (double)sumPriorities / sumHighestPriorities : 0;
	}
