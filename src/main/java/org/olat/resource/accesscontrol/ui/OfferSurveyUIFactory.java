/**
 * <a href="https://www.openolat.org">
 * OpenOLAT - Online Learning and Training</a><br>
 * <p>
 * Licensed under the Apache License, Version 2.0 (the "License"); <br>
 * you may not use this file except in compliance with the License.<br>
 * You may obtain a copy of the License at the
 * <a href="https://www.apache.org/licenses/LICENSE-2.0">Apache homepage</a>
 * <p>
 * Unless required by applicable law or agreed to in writing,<br>
 * software distributed under the License is distributed on an "AS IS" BASIS, <br>
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. <br>
 * See the License for the specific language governing permissions and <br>
 * limitations under the License.
 * <p>
 * Initial code contributed and copyrighted by<br>
 * frentix GmbH, https://www.frentix.com
 * <p>
 */
package org.olat.resource.accesscontrol.ui;

import java.util.List;

import org.olat.core.CoreSpringFactory;
import org.olat.core.gui.translator.Translator;
import org.olat.core.util.StringHelper;
import org.olat.resource.accesscontrol.ACService;
import org.olat.resource.accesscontrol.AccessControlModule;
import org.olat.resource.accesscontrol.Offer;
import org.olat.resource.accesscontrol.OfferAccess;
import org.olat.resource.accesscontrol.method.AccessMethodHandler;

/**
 *
 * Initial date: 8 Sep 2026<br>
 * @author uhensler, urs.hensler@frentix.com, https://www.frentix.com
 *
 */
public class OfferSurveyUIFactory {

	private OfferSurveyUIFactory() {
		//
	}
	
	public static String getOfferLabel(Offer offer, Translator translator) {
		if (StringHelper.containsNonWhitespace(offer.getLabel())) {
			return offer.getLabel();
		}
		
		ACService acService = CoreSpringFactory.getImpl(ACService.class);
		List<OfferAccess> offerAccesses = acService.getOfferAccess(offer, true);
		if (!offerAccesses.isEmpty()) {
			AccessControlModule acModule = CoreSpringFactory.getImpl(AccessControlModule.class);
			AccessMethodHandler handler = acModule.getAccessMethodHandler(offerAccesses.get(0).getMethod().getType());
			if (handler != null) {
				return handler.getMethodName(translator.getLocale());
			}
		}
		return translator.translate("offer.survey.offer.column");
	}

}
