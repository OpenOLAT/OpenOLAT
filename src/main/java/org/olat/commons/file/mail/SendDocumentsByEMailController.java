...
public class SendDocumentsByEMailController extends FormBasicController implements CmdSendMail {
	...
	protected void initForm(FormItemContainer formLayout, Controller listener, UserRequest ureq) {
		...
		((TextBoxListComponent)userListBox.getComponent()).setMapperProvider(new UserListProvider());
		((TextBoxListComponent)userListBox.getComponent()).setAllowNewValues(allowExtern);
		((TextBoxListComponent)userListBox.getComponent()).setAllowDuplicates(false);
		((TextBoxListComponent)userListBox.getComponent()).setMaxResults(MAX_RESULTS_USERS + 2);
	...
	protected void appendPublicationDate(MetaInfo infos, StringBuilder sb) {
		...
		if(publicationDate == null || publicationDate.length != 2) return;
	...
	protected boolean validateFormLogic(UserRequest ureq) {
		...
		List<String> values = this.userListBox.getValueList();
		if(FolderConfig.getSendDocumentToExtern()) {
			for(String value:values) {
				allOk &= (MailHelper.isValidEmailAddress(value) || securityManager.isIdentityVisible(value));
			}
		} else {
			for(String value:values) {
				allOk &= securityManager.isIdentityVisible(value);
			}
		}
	...
	public class UserListProvider implements ListProvider, ResultMapProvider {
		...
		public void getAutoCompleteContent(String searchValue, Map<String, String> resMap) {
			...
			if (StringHelper.containsNonWhitespace(searchValue)) {
				List<Identity> res = securityManager.getVisibleIdentitiesByPowerSearch(searchValue, userProperties, false,null, null, null, null, null);
