 ...
 *
 */
public class TagSelectionRenderer extends DefaultComponentRenderer {
	...
	public void renderComponent(Renderer renderer, StringOutput target, Component source, URLBuilder urlBuilder, Translator translator,
		...
		TagSelectionComponent cmp = (TagSelectionComponent)source;
		TagSelectionImpl el = cmp.getFormItem();
		FormLink button = el.getButton();
		
		if (el.isEnabled()) {
			renderer.render(button.getComponent(), target, null);
		} else {
			target.append("<span class=\"o_tag_selection_button\">");
