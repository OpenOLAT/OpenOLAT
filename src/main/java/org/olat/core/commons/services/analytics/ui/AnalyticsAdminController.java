...
/**
 * Admin user interface to configure the analytics service. Depending on the
 ...
 *
 */
public class AnalyticsAdminController extends BasicController {
	@Autowired
	private AnalyticsModule analyticsModule;

	private VelocityContainer mainVC;
	// the SPI chooser
	private AnalyticsSPIChooserFormController spiChooserCtr;
	 ...
	 */
	public AnalyticsAdminController(UserRequest ureq, WindowControl wControl) {
		super(ureq, wControl);
		mainVC = createVelocityContainer("analytics_module");
		putInitialPanel(mainVC);

		// 1) the SPI chooser
		spiChooserCtr = new AnalyticsSPIChooserFormController(ureq, wControl);
