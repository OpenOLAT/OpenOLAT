...
public class SitemapIndexWriter {
	...
	private static final Logger log = Tracing.createLoggerFor(SitemapIndexWriter.class);
	
	private final List<SitemapIndexItem> items;

	public SitemapIndexWriter(List<SitemapIndexItem> items) {
		this.items = items;
	}
	...
	private class XMLSitemap implements XmlWritable {
		...
		public void write(XmlWriter writer) throws XmlWriteException {
			try {
				...
				for (SitemapIndexItem item : items) {
					if (StringHelper.containsNonWhitespace(item.getLoc())) {
						writer.writeStartElement(NAMESPACE_URI, "sitemap");
