 ...
 */
public class StackedController extends DefaultController {
	
	private final List<Link> stack = new ArrayList<Link>(3);
	private final VelocityContainer mainVC;
	
	public StackedController(WindowControl wControl, Translator trans, String mainCssClass) {
		super(wControl);
	...
	public void pushController(String displayName, Controller controller) {
		...
		stack.add(link);
		mainVC.put("content", controller.getInitialComponent());
	}
	...
	protected void event(UserRequest ureq, Component source, Event event) {
		if(stack.contains(source)) {
			...
			if(index < (stack.size() - 1)) {
				...
				mainVC.put("content", currentCtrl.getInitialComponent());
				fireEvent(ureq, new PopEvent(popedCtrl));
			}
		}
	}
