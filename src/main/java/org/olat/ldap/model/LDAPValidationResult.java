 ...
 *
 */
public class LDAPValidationResult implements ValidationResult {
	
	private final boolean valid;
	private final List<ValidationDescription> errors = new ArrayList<>();
	
	private LDAPValidationResult(boolean valid) {
		this.valid = valid;
	}
	
	public static final ValidationResult allOk() {
		return new LDAPValidationResult(true);
	}
	
	public static final ValidationResult error(String i18nKey) {
		LDAPValidationResult result = new LDAPValidationResult(false);
		Translator translator = Util.createPackageTranslator(LDAPAuthenticationController.class, Locale.getDefault());
