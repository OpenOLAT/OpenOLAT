...
public class LDAPLoginManagerImpl implements LDAPLoginManager, AuthenticationProviderSPI, GenericEventListener {
	...
	public Identity authenticate(String login, String pwd, AuthenticationStatus status) {
		...
		if(StringHelper.containsNonWhitespace(login) && StringHelper.containsNonWhitespace(pwd)) {
			authenticatedIdentity = authenticate(login, pwd, ldapError);
	...
	public void event(Event event) {
		if(event instanceof LDAPEvent ldape) {
	...
	public Identity createAndPersistUser(Attributes userAttributes) {
		...
		final String uid = getSingleAttributeValue(userAttributes.get(syncConfiguration
				.getOlatPropertyToLdapAttribute(LDAPConstants.LDAP_USER_IDENTIFYER)));
		final String email = getSingleAttributeValue(userAttributes.get(syncConfiguration.getOlatPropertyToLdapAttribute(UserConstants.EMAIL)));
		if(!validate(uid, email)) {
			 return null;
		}
		 
		// Create User (first and lastname is added in next step)
		User user = userManager.createUser(null, null, email);
		if(!applyAttributes(uid, user, userAttributes)) {
			return null;
		}

		// Create Identity and add it to the default organization
		final String identityName = securityModule.isIdentityNameAutoGenerated() ? null : uid;
		Identity identity;
		if(syncConfiguration.syncOrganisationWithLDAPGroup()) {
	...
	public TransientIdentity createTransientIdentity(Attributes userAttributes) {
		...
		// Create user (first and last name is added in next step)
		TransientIdentity identity = new TransientIdentity();
		identity.setLdap(true);
		identity.setName(uid);
		// Set User Property's (Iterates over Attributes and gets OLAT Property)
		if(applyAttributes(uid, identity.getUser(), userAttributes)) {
			return identity;
		}
		return null;
	}
	
	private boolean applyAttributes(String uid, User user, Attributes userAttributes) {
	...
	private boolean validate(String uid, String email) {
		// Lookup user
	...
	throws InvalidAttributeValueException {
		if(attrs == null) {
			errors.insert("findIdentyByLdapAuthentication: attrs::null 2");
			return null;
		}
		
		final String uidAttribute = syncConfiguration.getOlatPropertyToLdapAttribute(LDAPConstants.LDAP_USER_IDENTIFYER);
		final List<String> loginAttributes = syncConfiguration.getLdapUserLoginAttributes();
		final String uid = getSingleAttributeValue(attrs.get(uidAttribute));
		...
		for(String loginAttribute:loginAttributes) {
			...
			if(StringHelper.containsNonWhitespace(loginToken)) {
				...
				if(ldapAuth != null) {
					return updateAuthentication(loginToken, uid, ldapAuth);
				}
		...
		Authentication ldapAuth = authenticationDao.getAuthenticationByExternalId(uid, LDAPAuthenticationController.PROVIDER_LDAP, BaseSecurity.DEFAULT_ISSUER);
		if(ldapAuth == null) {
			ldapAuth = authenticationDao.getAuthentication(uid, LDAPAuthenticationController.PROVIDER_LDAP, BaseSecurity.DEFAULT_ISSUER);
		}
		if(ldapAuth != null) {
			return updateAuthentication(token, uid, ldapAuth);
		}
		...
		if(ldapLoginModule.isConvertExistingLocalUsersToLDAPUsers()) {
			...
			if(defaultAuth != null) {
				...
				securityManager.createAndPersistAuthentication(defaultAuth.getIdentity(), LDAPAuthenticationController.PROVIDER_LDAP, BaseSecurity.DEFAULT_ISSUER, null,
						token, null, null);
			...
			if(identity != null) {
				ldapAuth = securityManager.createAndPersistAuthentication(identity, LDAPAuthenticationController.PROVIDER_LDAP, BaseSecurity.DEFAULT_ISSUER, null,
						token, null, null);
	...
	private Authentication updateAuthentication(String authusername, String externalId, Authentication ldapAuth) {
		boolean update = false;
		if(StringHelper.containsNonWhitespace(authusername) && !authusername.equals(ldapAuth.getAuthusername())) {
			ldapAuth.setAuthusername(authusername);
			update = true;
		}
		if(StringHelper.containsNonWhitespace(externalId) && ldapAuth.getExternalId() == null) {
			ldapAuth.setExternalId(externalId);
			update = true;
		}
