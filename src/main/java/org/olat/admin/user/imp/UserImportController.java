			...
			try {
				if (runContext.containsKey("validImport") && ((Boolean) runContext.get("validImport")).booleanValue()) {
					...
					for (TransientIdentity newIdent:newIdents) {
						...
						try {
							if (sendNewPasswords) {
								MailerResult result = util.sendTokenByMail(ureq, newIdentity, util.generateMailText(newIdentity));
								if (!result.isSuccessful()) {
									report.addError(translate("sendmail.passwordchange.failed", newIdentity.getName())
