...
public class AiQtiItemFactory {
	 ...
	 * scoring, max score 1.0 and a "show solution" modal feedback.
	 * <p>
	 ...
	 * @param solutionTitle translated "Solution" label, used by the modal feedback header
	...
	public McItem buildMcItem(MCQuestionData data, Locale locale, String solutionTitle,
		...
		// correct answers
		for (String correctValue : data.getCorrectAnswers()) {
			if (!StringHelper.containsNonWhitespace(correctValue)) continue;
			String safe = StringHelper.xssScan(correctValue);
		...
		if (data.getWrongAnswers() != null) {
			for (String wrongValue : data.getWrongAnswers()) {
				if (!StringHelper.containsNonWhitespace(wrongValue)) continue;
				String safe = StringHelper.xssScan(wrongValue);
				ChoiceInteraction interaction = mcBuilder.getChoiceInteraction();
				SimpleChoice newChoice = AssessmentItemFactory.createSimpleChoice(interaction, safe,
	...
	public EssayItem buildEssayItem(EssayItemDraft draft, Locale locale, String solutionTitle,
		...
		builder.setQuestion(stimulusToHtml(draft == null ? null : draft.stimulus()));
	...
	private static String mcSolutionHtml(List<String> correctAnswers) {
		StringBuilder sb = new StringBuilder();
		sb.append("<ul>");
		for (String a : correctAnswers) {
			if (!StringHelper.containsNonWhitespace(a)) continue;
			sb.append("<li>").append(escapeXml(a.trim())).append("</li>");
		}
		sb.append("</ul>");
		return sb.toString();
	}
