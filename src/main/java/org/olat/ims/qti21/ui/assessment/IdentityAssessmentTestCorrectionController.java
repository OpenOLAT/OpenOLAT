...
import org.olat.ims.qti21.ui.AssessmentTestHelper;
...
public class IdentityAssessmentTestCorrectionController extends FormBasicController {
	...
	protected void initForm(FormItemContainer formLayout, Controller listener, UserRequest ureq) {
		...
		uifactory.addFormCancelButton("cancle", buttonsCont, ureq, getWindowControl());
	}
	...
	private void initFormItemResult(FormLayoutContainer layoutCont, TestPlanNode node,
		...
		for(Interaction interaction:interactions) {
			if(interaction instanceof UploadInteraction
				...
				InteractionResultFormItem responseItem = initFormExtendedTextInteraction(resolvedAssessmentItem, testPlanNodeKey, interaction, layoutCont);
		...
		if(responseItems.size() > 0) {
			...
			if(identifierToItemSession.containsKey(stringuifiedIdentifier)) {
				AssessmentItemSession itemSession = identifierToItemSession.get(stringuifiedIdentifier);
			...
			IdentityAssessmentItemWrapper wrapper = new IdentityAssessmentItemWrapper(assessmentItem, testPlanNodeKey, responseItems, scoreEl);
