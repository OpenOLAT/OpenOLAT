...
public abstract class AssessmentObjectComponentRenderer extends DefaultComponentRenderer {
	...
	protected void renderItemStatus(AssessmentRenderer renderer, StringOutput sb, ItemSessionState itemSessionState, RenderingRequest options, Translator translator) {
		if(options != null && options.isSolutionMode()) {
			renderItemStatusMessage(renderer, "review", "assessment.item.status.modelSolution", sb, translator);
		} else if(options != null && options.isReviewMode()) {
			renderItemReviewStatus(renderer, sb, itemSessionState, translator);
	...
	protected void renderItemReviewStatus(AssessmentRenderer renderer, StringOutput sb, ItemSessionState itemSessionState, Translator translator) {
	...
	protected void renderItemStatusMessage(AssessmentRenderer renderer, String status, String i18nKey, StringOutput sb, Translator translator) {
		...
		sb.append("<span class='o_assessmentitem_status ");
		if (renderer.isPageMode()) {
			sb.append("o_labeled ");
		}
		sb.append(status).append(" ' title=\"").append(StringHelper.escapeHtml(title))
	...
	public void renderDlElement(AssessmentRenderer renderer, StringOutput sb, AssessmentObjectComponent component,
		...
		switch(dlElement.getQtiClassName()) {
			case Dt.QTI_CLASS_NAME:
				((Dt)dlElement).getInlines().forEach(inline -> renderInline(renderer, sb, component, resolvedAssessmentItem, itemSessionState, inline, ubu, translator));
				break;
			case Dd.QTI_CLASS_NAME:
				((Dd)dlElement).getFlows().forEach(flow -> renderFlow(renderer, sb, component, resolvedAssessmentItem, itemSessionState, flow, ubu, translator));
				break;
			default:
				//ignore other type
				break;
		...
		switch(inline.getQtiClassName()) {
			...
			case FeedbackInline.QTI_CLASS_NAME: {
				...
				if(component.isFeedback(feedbackInline, itemSessionState)) {
					...
					feedbackInline.getInlines().forEach(child -> renderInline(renderer, sb, component, resolvedAssessmentItem, itemSessionState, child, ubu, translator));
					sb.append("</span>");
				}
				break;
			}
			...
			default: {
				...
				if(inline instanceof SimpleInline) {
					...
					simpleInline.getInlines().forEach(child -> renderInline(renderer, sb, component, resolvedAssessmentItem, itemSessionState, child, ubu, translator));
				}
	...
	protected final void renderSpan(AssessmentRenderer renderer, StringOutput sb, Span span, AssessmentObjectComponent component,
		...
		span.getInlines().forEach(child -> renderInline(renderer, sb, component, resolvedAssessmentItem, itemSessionState, child, ubu, translator));
	...
	protected final void renderA(AssessmentRenderer renderer, StringOutput sb, A a, AssessmentObjectComponent component,
		...
		a.getInlines().forEach(child -> renderInline(renderer, sb, component, resolvedAssessmentItem, itemSessionState, child, ubu, translator));
		renderEndTag(sb, a);
	}
	...
	protected final void renderInfoControl(AssessmentRenderer renderer, StringOutput sb, AssessmentObjectComponent component,
		...
		infoControl.getChildren().forEach(flow -> renderFlow(renderer, sb, component, resolvedAssessmentItem, itemSessionState, flow, ubu, translator));
		sb.append("</div></div>");
	}
	...
	protected final void renderExtendedTextBox(AssessmentRenderer renderer, StringOutput sb, AssessmentObjectComponent component, AssessmentItem assessmentItem,
		...
		if(ended) {
			...
			sb.append(FormJSHelper.getJSStart())
			  ...
			  .append("  errorMessage:'").append(translator.translate("error.not.saved")).append("'\n")
			  .append(" })");
