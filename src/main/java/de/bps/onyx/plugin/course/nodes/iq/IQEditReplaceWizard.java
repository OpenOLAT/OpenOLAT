...
public class IQEditReplaceWizard extends WizardController {
	 ...
	 * @param courseNode
	 */
	public IQEditReplaceWizard(UserRequest ureq, WindowControl wControl, ICourse course, CourseNode courseNode, String[] types, List<Identity> learners, List<QTIResult> results, int numberOfQtiSerEntries, boolean isOnyx) {
	...
	private void doStep2Onyx(UserRequest ureq) {
		...
		if (courseNode.getClass().equals(IQSURVCourseNode.class)) {
			...
			String repositorySoftKey = (String) courseNode.getModuleConfiguration().get(
