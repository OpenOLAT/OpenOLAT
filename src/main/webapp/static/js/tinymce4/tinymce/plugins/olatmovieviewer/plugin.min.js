...
(function() {
	...
	function previewIframe(editor) {
		...
		node = iframeWindow.document.createElement('script');
		node.src = contextPath + '/raw/_noversion_/js/jquery/jquery-3.7.0.min.js';
		node.type = 'text/javascript';
		node.async = true;
		node.charset = 'utf-8';
		iframeHead.appendChild(node);
	...
	function getPlayerHtmlNode(editor, p) {
		...
		var node;
		var extendedValidElements = editor.getParam("extended_valid_elements");
		if(extendedValidElements != null && extendedValidElements.indexOf('script') >= 0) {
			...
			h += '</script>';
			node = editor.dom.create("span", {
				id:domIdentity,
				name:domIdentity,
				"class":"olatFlashMovieViewer",
				"style":'display:block; border:solid 6px #000; width:' + playerWidth + 'px; height:' + playerHeight + 'px;'
			},h);
		} else {
			// <object id="olatFlashMovieViewer1311" class="olatFlashMovieViewer" data="demo-video.mp4" type="video" width="320" height="240" data-oo-movie="'demo-video.mp4','olatFlashMovieViewer1311',320,240,'',0,'video',undefined,false,false,true,''"></object>
			...
			node = editor.dom.create("object", {
				id:domIdentity,
				name:domIdentity,
				"class":"olatFlashMovieViewer",
				"data": p.address.value,
				"type": "video",
				"width": playerWidth,
				"height": playerHeight,
				"data-oo-movie": "'" + p.address.value + "','" + domIdentity + "'," + playerWidth + "," + playerHeight + "," + starttime + ",0," + provider + "," + streamer + "," + autostart + "," + repeat + "," + controlbar + "," + poster
			}," ");
		}
	...
	var register = function (editor, url) {
		var onAction = function() {
			...
			var onSubmit = function (dialog) {
				...
				var sound = (data.provider == "sound");
				data.width = data.width == "" ? (sound ? 300 : 640) : data.width;
				data.height = data.height == "" ? (sound ? 40 : 480) : data.height;
			...
			var onChange = function (dialogApi, details) {
				try {
					...
					if(details.name === "address") {
						...
						if(data.provider === "nanoo" || (data.provider === "video" && data.address.value.indexOf("https://nanoo.tv") == 0)) {
		...
		var onRecordAudioAction = function() {
			let id = editor.getParam('id');
			BTinyHelper.recordAudio(id, recordAudioCallback, '', { 'filetype': 'audio' });
		};

		var recordAudioCallback = function(link, info) {
			var movieParameters = {
				provider: 'sound',
				address: { value: link },
				width: 300,
			...
			var serializedMovieParameters = JSON.stringify(movieParameters);

			var fe = editor.selection.getNode();
			if (fe && /mceItemOlatMovieViewer/.test(editor.dom.getAttrib(fe, 'class'))) {
				fe.setAttribute("data-oo-movie-parameters", serializedMovieParameters);
				fe.style.width = `${movieParameters.width}px`;
				fe.style.height = `${movieParameters.height}px`;
				fe.style.backgroundColor = "#ffffcc";
				fe.style.backgroundImage = "url('" + url + "/css/olatmovieviewer.png')";
				fe.style.backgroundPosition = "center";
				fe.style.backgroundSize = "32px 32px";
				...
				var h = "<img id='" + newDomId + "'";
				h += " class='mceItemOlatMovieViewer' src='" + editor.getParam("olatmovieviewer_transparentImage") + "'";
				h += " data-oo-movie-parameters='" + JSON.stringify(movieParameters) + "' data-mce-placeholder='1'";
				h += ` width='${movieParameters.width}' height='${movieParameters.height}' style='width: ${movieParameters.width}px; height: ${movieParameters.height}px; '`;
				h += "/>";
				editor.execCommand("mceInsertContent", false, h);
			}
		}

		editor.ui.registry.addToggleButton('olatrecordaudio', {
			icon : 'oo-record-audio',
			tooltip: translator().translate('olatmovieviewer.record_audio'),
			onAction: onRecordAudioAction
		});

		editor.ui.registry.addMenuItem('olatrecordaudio', {
			text: translator().translate('olatmovieviewer.record_audio'),
			icon: 'oo-record-audio',
			context: 'insert',
			onAction: onRecordAudioAction
		});
