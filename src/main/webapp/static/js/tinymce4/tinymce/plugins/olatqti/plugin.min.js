    ...
    var register = function (editor, url) {
		...
		function showDialog(e, gapType) {
			...
			if(typeof lastSelectedGap != 'undefined') {
				...
				o_ffXHREvent(ffxhrevent.formNam, ffxhrevent.dispIdField, ffxhrevent.dispId, ffxhrevent.eventIdField, 2, false, false, false, true,
				...
				let placeholder = createTextEntryPlaceholder(responseIdentifier, newSelectedText, 'textentryinteraction', gapType);
				let holderHtml = tinymce.html.Serializer().serialize(placeholder);
		...
		function createHottext(e) {
			...
			if(typeof lastSelectedHottext != 'undefined') {
				...
				let identifier = "ht" + guid();
				let placeholder = createHottextPlaceholder(identifier, selectedText, false, 'hottext');
				let holderHtml = tinymce.html.Serializer().serialize(placeholder);
				editor.insertContent(holderHtml + " ");
		...
		function showInlineChoiceDialog(e) {
			...
			if(typeof lastSelectedInlineChoice != 'undefined') {
				...
				o_ffXHREvent(ffxhrevent.formNam, ffxhrevent.dispIdField, ffxhrevent.dispId, ffxhrevent.eventIdField, 2, false, false, true, true,
						'_csrf', ffxhrevent.csrf, 'qcmd', 'inlinechoiceinteraction', 'responseIdentifier', responseIdentifier, 'newEntry', false);
			} else {
				let nextCounter = nextResponseNumber();
				let responseIdentifier = "inline" + guid();
				...
				let placeholder = createInlineChoicePlaceholder(responseIdentifier, selectedText, 'inlinechoiceinteraction');
				let holderHtml = tinymce.html.Serializer().serialize(placeholder);
				editor.insertContent(holderHtml);
		...
		function createInlineChoice(e) {
			...
			if(typeof lastSelectedInlineChoice != 'undefined') {
				...
				let identifier = "inline" + guid();
				let placeholder = createInlineChoicePlaceholder(identifier, selectedText, 'inlinechoiceinteraction');
				let holderHtml = tinymce.html.Serializer().serialize(placeholder);
				editor.insertContent(holderHtml);
				
				if(preSelect) {
					let selectedNode = editor.dom.select("span[data-qti-identifier=" + identifier + "] span[contenteditable=true]");
					editor.selection.select(selectedNode[0], true);
				}
			}
		}
		...
		function isNodeAlone(node) {
			return (node.previousSibling == null
					|| (node.previousSibling != null && node.previousSibling.textContent != null
							&& (node.previousSibling.textContent == ""
								|| (node.previousSibling.textContent.length == 1 && node.previousSibling.textContent.charCodeAt(0) == 65279))))
		...
		editor.on('NodeChange', function(e) {
			...
			if (lastSelectedInlineChoice && lastSelectedInlineChoice.id != e.element.src) {
				lastSelectedInlineChoice = undefined;
			}
			...
			} else if (jQuery(e.element).parent("span[data-qti='inlinechoiceinteraction']").length > 0) {
				lastSelectedInlineChoice = e.element;
			}
		...
		function createTextEntryPlaceholder(responseIdentifier, content, interaction, gapType) {
			...
			if("&nbsp;" === content) {
				...
				escapedContent = jQuery("<div>").text(content).html();
			}
		...
		function replaceTextContent(content) {
			...
			jQuery(htmlContent).find("span[data-qti='textentryinteraction']").each(function(index, el) {
				let sourceEntryId = jQuery(el).attr('entryId');
				let entryId = 'te' + guid();
				jQuery(el).attr('data-qti-response-identifier', entryId);
				jQuery(el).attr('data-copy', 'needlistener');
				jQuery(el).attr('data-copy-empty', 'false');
				
				let gapType = jQuery(el).attr("data-qti-gap-type");
				let solution = jQuery(el).attr("data-qti-solution");
				let ffxhrevent = editor.getParam("ffxhrevent");
				o_ffXHRNFEvent(ffxhrevent.formNam, ffxhrevent.dispIdField, ffxhrevent.dispId, ffxhrevent.eventIdField, 2,
						'_csrf', ffxhrevent.csrf, 'cmd', 'copy-gapentry', 'responseIdentifier', entryId, 'sourceResponseIdentifier', sourceEntryId, 'newEntry', true, 'selectedText', solution, 'gapType', gapType);
				//add it because tiny delete it
				jQuery("a.o_ops", el).append(jQuery("<i class='visible'>&nbsp;</i>"));
				
				let empty = jQuery("span[contenteditable='true']", el).text();
				if(empty == null || empty.length == 0) {
					jQuery("span[contenteditable='true']", el).text("x-y-x");
					jQuery(el).attr('data-copy-empty', 'true');
				}
			...
			jQuery(htmlContent).find("textentryinteraction").each(function(index, el) {
				let sourceEntryId = jQuery(el).attr('responseidentifier');
				let entryId = 'te' + guid();
			...
			jQuery(htmlContent).find("inlinechoiceinteraction").each(function(index, el) {
				let sourceResponseIdentifier = jQuery(el).attr('responseidentifier');
				let entryId = 'te' + guid();
				jQuery(el).attr('responseidentifier', entryId);
				let ffxhrevent = editor.getParam("ffxhrevent");
				
				o_ffXHRNFEvent(ffxhrevent.formNam, ffxhrevent.dispIdField, ffxhrevent.dispId, ffxhrevent.eventIdField, 2,
						'_csrf', ffxhrevent.csrf, 'qcmd', 'copy-inlinechoice', 'responseIdentifier', entryId, 'newEntry', true, 'sourceResponseIdentifier', sourceResponseIdentifier);
				replace = true;
			});
		...
		editor.on('init', function() {
			let contentCss = editor.getParam("content_css");
			if (contentCss !== false) {
			...
			editor.parser.addNodeFilter('textentryinteraction,hottext,inlinechoiceinteraction', function(nodes) {
				for (let i=0; i<nodes.length; i++) {
					...
					if (node.name == 'textentryinteraction') {
						let responseIdentifier = node.attr('responseidentifier');
						if(responseIdentifier.lastIndexOf("RESPONSE_", 0) == 0) {
							let id = parseInt(responseIdentifier.substring(9, responseIdentifier.length));
							if(id > lastResponseIdentifierCounter) {
								lastResponseIdentifierCounter = id;
							}
						}

						let solution = node.attr('data-qti-solution');
						if(typeof solution === "undefined") {
							solution = "&nbsp;";
						}
						let gapType = node.attr('data-qti-gap-type');
						if(typeof gapType === "undefined") {
							gapType = "string";
						}
						
						let placeHolder = createTextEntryPlaceholder(responseIdentifier, solution, 'textentryinteraction', gapType);
						let replaced = node.replace(placeHolder);
						placeHolder.next = node.firstChild;
					} else if (node.name == 'hottext') {
						let identifier = node.attr('identifier');
						let correctHottexts = editor.getParam("correctHottexts");
						let correct = jQuery.inArray(identifier, correctHottexts) >= 0;
						let content = node;
						
						let placeHolder = createHottextPlaceholder(identifier, content, correct, 'hottext', 'hottext');
						node.replace(placeHolder);
					} else if (node.name == 'inlinechoiceinteraction') {
						let responseIdentifier = node.attr('responseidentifier');
						let solution = node.attr('data-qti-solution');
						let correctResponses = editor.getParam("correctResponses");
						let missingCorrectResponses = editor.getParam("missingCorrectResponses");
						let missing = jQuery.inArray(responseIdentifier, missingCorrectResponses) >= 0;
						
						if(missing) {
							solution = translator().translate('warning.correct.answer.short');
						} else if(correctResponses != null) {
							for(let j=correctResponses.length; j-->0; ) {
						...
						let content = node;
						let placeHolder = createInlineChoicePlaceholder(responseIdentifier, solution, 'inlinechoiceinteraction');
						node.replace(placeHolder);
					}
				}
			});
		...
		editor.on('PreProcess', function(e) {
			jQuery("span[data-qti='textentryinteraction']", e.node).each(function(idx, node) {
				let jNode = jQuery(node);
				let identifier = jNode.attr('data-qti-response-identifier');
				let solution = jNode.children('span').text();
				let gapType = jNode.attr('data-qti-gap-type');
				let textNode = editor.dom.create("textEntryInteraction", {
		    ...
		    jQuery("span[data-qti='inlinechoiceinteraction']", e.node).each(function(idx, node) {
				...
				var alone = isNodeAlone(node);
				editor.dom.replace(inlineChoiceNode, node, false);
				if(alone) {
					jQuery(inlineChoiceNode).after(String.fromCharCode(160));
				}
		    });
		});
