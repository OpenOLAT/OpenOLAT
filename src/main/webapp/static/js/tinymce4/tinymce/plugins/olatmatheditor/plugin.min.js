...
(function() {
	...
	var register = function (editor, url) {
		...
		const mathJaxConfiguration = function() {
			return {
				tex: {
					inlineMath: [['$$', '$$'], ['\\(', '\\)']],
					macros: {
				...
				loader: {
					paths: {
						fonts: editor.getParam("mathJaxUrl")
					}
				},
				output: {
					font: 'mathjax-newcm',
					fontPath: editor.getParam("mathJaxUrl") + '%%FONT%%-font'
				}
			};
		}
		...
		function showDialog() {
			...
			const mathJaxUrl = editor.getParam("mathJaxUrl");
			const mathLiveUrl = editor.getParam("mathLiveUrl");
			const previewSrc = editor.getParam("contextPath") + '/olat/raw/_noversion_/js/tinymce4/preview.html';
			...
			let body = {
				...
				items: [
				    ...
				    { name: 'preview', type: 'htmlpanel', label: '',
				    	html: '<iframe id="mathpreviewFormula" src="' + previewSrc + '" style="width: 100%; min-height: 50px;"></iframe>'
 					}
				]
			};
			...
			var win = editor.windowManager.open({
		        ...
		        onChange: function(dialogApi) {
					...
					mf.setValue(dialogApi.getData().latex, {silenceNotifications:true,suppressChangeNotifications:true});
			...
			function updatePreview(dialogApi) {
				...
				if(iframeWindow.MathJax === undefined) {
					...
					const scriptConfig = editor.dom.create('script', { id: id + 'Config'}, "MathJax = " + JSON.stringify(mathJaxConfiguration()));
					const iframeHeadElement = iframeDocument.getElementsByTagName('head')[0];
					iframeHeadElement.appendChild(scriptConfig);
					const script = editor.dom.create('script', { id: id, type: 'text/javascript', src: 'https:' + mathJaxUrl + 'tex-mml-chtml.js' });
				...
				if(tex == null || tex.length == 0) {
					...
					emptyHtml += "<div class='o_empty_msg' style='font-family: sans-serif;'>" + translator().translate('olatmatheditor.empty.title') + "</div>";
		...
		editor.on('init', function() {
			...
			const id = editor.dom.uniqueId();
			const mathJaxUrl = editor.getParam("mathJaxUrl") + 'tex-mml-chtml.js';
			let found = false;
			for (let j = 0; j < scripts.length; j++) {
				if (scripts[j].src == mathJaxUrl) {
					found = true;
					break;
				}
			}
			// Insert the configuration and load script
			if (!found) {
				const config = "MathJax = " + JSON.stringify(mathJaxConfiguration());
				const scriptConfig = editor.dom.create('script', { id: id + 'Config'}, config);
				const headElement = editor.getDoc().getElementsByTagName('head')[0];
				headElement.appendChild(scriptConfig);
				const script = editor.dom.create('script', { id: id, type: 'text/javascript', src: mathJaxUrl });
