...
function(q){jQuery(g).removeClass("oo-accepted")}).on("out",function(q){jQuery(g).removeClass("oo-accepted")});t(b,d);h(b,d,c)}function t(a,c){a.find("li").on("click",function(b,d){p(jQuery(this),a,c)})}function p(a,c,b){a.hasClass("oo-selected")?(l(c,b),b.removeClass("oo-selected")):(l(c,b),a.addClass("oo-selected"),b.addClass("oo-selected"))}function h(a,c,b){c.on("click",function(d,e){d.target===c.get(0)?(d=a.find("li.oo-selected"),0!==d.length&&(d.removeClass("oo-selected"),e=d.clone(),c.append(e),
c.removeClass("oo-selected"),d.remove(),f(b),setFlexiFormDirty(b.formDispatchFieldId))):m(d.target,a,c,b)})}function m(a,c,b,d){a=jQuery(a).closest("li.o_assessmentitem_order_item");if(0<a.length){l(c,b);b.removeClass("oo-selected");var e=a.clone();e.on("click",function(g,q){p(jQuery(this),c,b)});c.append(e);a.remove();f(d);setFlexiFormDirty(d.formDispatchFieldId)}}function l(a,c){c.find("li").each(function(b,d){jQuery(d).removeClass("oo-selected")});a.find("li").each(function(b,d){jQuery(d).removeClass("oo-selected")})}
