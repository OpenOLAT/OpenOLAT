...
c.removeClass("oo-selected"),d.remove(),f(b))):m(d.target,a,c,b)})}function m(a,c,b,d){a=jQuery(a).closest("li.o_assessmentitem_order_item");if(0<a.length){l(c,b);b.removeClass("oo-selected");var e=a.clone();e.on("click",function(g,q){p(jQuery(this),c,b)});c.append(e);a.remove();f(d)}}function l(a,c){c.find("li").each(function(b,d){jQuery(d).removeClass("oo-selected")});a.find("li").each(function(b,d){jQuery(d).removeClass("oo-selected")})}function f(a){var c="#qtiworks_response_"+a.responseIdentifier;
jQuery(c+" div.source \x3e ul");var b=jQuery(c+" div.target \x3e ul"),d=jQuery(c+" div.target"),e=jQuery(c+" div.hiddenInputContainer"),g=b.children("li").length;if(null!=a.minChoices&&null!=a.maxChoices){if(g<a.minChoices||g>a.maxChoices)return a.minChoices!=a.maxChoices?alert("You must select and order between "+a.minChoices+" and "+a.maxChoices+" items"):alert("You must select and order exactly "+a.minChoices+" item"+(1<minChoices?"s":"")),d.toggleClass("highlight",!0),!1;d.toggleClass("highlight",
!1)}e=jQuery(c+" div.hiddenInputContainer");e.empty();b.children("li").each(function(q){q=this.id.substring(18);var r=jQuery('\x3cinput type\x3d"hidden"\x3e');r.attr("name","qtiworks_response_"+a.responseIdentifier);r.attr("value",q);e.append(r)})}v.fn.orderInteraction=function(a){a=v.extend({responseIdentifier:null,formDispatchFieldId:null,minChoices:null,maxChoices:null,responseValue:null,opened:!1},a);try{"undefined"!=typeof a.responseValue&&0<a.responseValue.length&&f(a),a.opened&&w(this,a)}catch(c){window.console&&
console.log(c)}return this}})(jQuery);
