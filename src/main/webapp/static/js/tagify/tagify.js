            ...
            case 'Tab':
              {
                try {
                  ...
                  if(this.input.value != null && this.input.value.indexOf('a:') == 0 && value != null && value.indexOf('a:') < 0) {
                	  value = 'a:' + value;
                  }
    ...
    filterListItems: function filterListItems(value) {
      ...
      if (!value) {
        return whitelist.filter(function (item) {
          return !_this10.isTagDuplicate(item.value || item) && item.searchBy.indexOf('a:') < 0;// remove admin. references
      ...
      for (; i < whitelist.length; i++) {
        ...
        searchBy = (((whitelistItem.searchBy || '') + ' ' + whitelistItem.value).toLowerCase()).trim();
        ...
        if (valueIsInWhitelist && !isDuplicate && suggestionsCount--) {
        	if((value == '' && searchBy.indexOf('a:') >= 0)
        			|| (value.indexOf('a:') == 0 && whitelistItem.searchBy.indexOf('a:') < 0) // search with a:, remove not administrative tags
        			|| (value.indexOf('a:') < 0 && whitelistItem.searchBy.indexOf('a:') >= 0)) { // search with a:, remove administrative tags
        		continue;
        	}
