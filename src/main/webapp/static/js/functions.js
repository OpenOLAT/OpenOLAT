...
function o_onXHRError(jqXHR, textStatus, errorThrown, wasFlexiDirtyDirty) {
	o_afterserver();
	if(401 == jqXHR.status
			|| 403 == jqXHR.status // Typically CSRF mismatch
...
function o_isLoginPage(jqXHR) {
	try {
		...
		return (jqXHR.responseText != null) && (typeof jqXHR.responseText !== "undefined")
			&& (jqXHR.responseText.indexOf('o_login_box_wrapper') >= 0 || jqXHR.responseText.indexOf(">Reload<") >= 0);
	} catch(e) {
		if(window.console) console.log(e);
		return false;
	}
