...
update o_user set u_title=(select propvalue from o_userproperty where user_id=fk_user_id and propname='title') where u_title is null;
...
alter table o_userproperty drop constraint if exists FK4B04D83FD1A80C95;
...
update o_user set u_nickname=(select name from o_bs_identity where user_id=fk_user_id) where u_nickname is null;
...
-- views
drop view if exists o_bs_identity_short_v;
