...
create table o_noti_pub (
   ...
   fk_root_publisher int8,
   fk_parent_publisher int8,
...
create table o_repositoryentry (
   ...
   fk_lifecycle int8,
   lifecycle_over_eval_date timestamp,
...
create table o_cer_certificate (
   ...
   c_generation_retries int8 default 0 not null,
   c_generation_next_date timestamp,
   c_generation_data text,
...
create table o_vid_task_session (
   ...
   v_segments int8 default 0 not null,
...
create table o_eva_form_participation (
   ...
   e_run int8 default 1,
   e_last_run bool not null default true,
   e_email varchar(128),
...
create table o_gta_task (
   ...
   g_collection_revisions_date timestamp,
   g_collection_revisions_ndocs int8,
   ...
   g_peerreview_due_date timestamp,
   g_peerreview_completed_date timestamp,
...
create table o_jup_hub (
   ...
   j_cpu_guarantee int8,
   j_cpu int8 not null,
   j_additional_fields text,
...
create table o_tb_audit_log (
   ...
   fk_topic int8,
   fk_selection int8,
   fk_custom_field_definition int8,
   primary key (id)
);
...
create unique index idx_eva_surv_ppident_idx on o_eva_form_survey (e_public_part_identifier) where e_public_part_identifier is not null;
...
alter table o_feed add f_canrate bool default true;
alter table o_feed add f_cancomment bool default true;
