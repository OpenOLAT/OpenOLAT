...
public class CoachingDAOTest extends OlatTestCase {
	...
	public void getStatisticsDuplicateInGroups() {
		...
		// Courses statistics
		CoursesStatisticsParams coursesStatisticsParams = CoursesStatisticsParams.valueOf(CoursesStatisticsRuntimeTypesGroup.standaloneAndCurricular);
	...
	public void getStatisticsOwner() {
		...
		List<CourseStatEntry> coursesStatisticsAsOwner = coachingDAO.getCoursesStatistics(coach,
				GroupRoles.owner, coursesStatisticsParams);
		Assertions.assertThat(coursesStatisticsAsOwner)
			.hasSize(2);
		//re 1
		CourseStatEntry entryCourse1 = getCourseStatEntry(re1, coursesStatisticsAsOwner);
		...
		//re 2
		CourseStatEntry entryCourse2 = getCourseStatEntry(re2, coursesStatisticsAsOwner);
		...
		Assertions.assertThat(coursesStatisticsAsCoach)
			.hasSize(1);
		//re 3
		CourseStatEntry entryCourse3 = getCourseStatEntry(re3, coursesStatisticsAsCoach);
		Assert.assertNotNull(entryCourse3);
		Assert.assertEquals(0, entryCourse3.getParticipants());
		Assert.assertEquals(0, entryCourse3.getParticipantsVisited());
		Assert.assertEquals(0, entryCourse3.getSuccessStatus().numPassed());
		Assert.assertEquals(0, entryCourse3.getSuccessStatus().numFailed());
		Assert.assertEquals(0, entryCourse3.getSuccessStatus().numUndefined());
		Assert.assertNull(entryCourse3.getAverageScore());
		...
		SearchParticipantsStatisticsParams participantsSearch = SearchParticipantsStatisticsParams.as(coach, GroupRoles.owner)
				.withCourseCompletion(true)
				.withCourseStatus(true);
		List<ParticipantStatisticsEntry> usersStatistics = coachingService.getParticipantsStatistics(participantsSearch, userPropertyHandlers, Locale.ENGLISH);
		
		Assert.assertNotNull(usersStatistics);
		Assert.assertEquals(4, usersStatistics.size());
		
		// Participant1 is only in re 1
		ParticipantStatisticsEntry entryParticipant1 = getParticipantStatisticsEntry(participant1, usersStatistics);
		Assert.assertNotNull(entryParticipant1);
		Assert.assertEquals(1, entryParticipant1.getSuccessStatus().numPassed());
		Assert.assertEquals(0, entryParticipant1.getSuccessStatus().numFailed());
		Assert.assertEquals(0, entryParticipant1.getEntries().numOfNotVisited());
		...
		ParticipantStatisticsEntry entryParticipant2 = getParticipantStatisticsEntry(participant2, usersStatistics);
		Assert.assertNotNull(entryParticipant2);
		Assert.assertEquals(0, entryParticipant2.getSuccessStatus().numPassed());
		Assert.assertEquals(1, entryParticipant2.getSuccessStatus().numFailed());
		Assert.assertEquals(1, entryParticipant2.getEntries().numOfEntries());
		...
		ParticipantStatisticsEntry entryParticipant3 = getParticipantStatisticsEntry(participant3, usersStatistics);
		...
		ParticipantStatisticsEntry entryParticipant4 = getParticipantStatisticsEntry(participant4, usersStatistics);
	...
	public void getCoursesStatisticsEmptyStatementsEmptyCourseInfos() {
		...
		Assertions.assertThat(nativeCourseStatsAsOwner)
			.hasSize(1);
		//re 1
		CourseStatEntry entryCourse1 = getCourseStatEntry(re1, nativeCourseStatsAsOwner);
		Assert.assertNotNull(entryCourse1);
	...
	@Test
	public void isMemberByOwner() throws URISyntaxException {
		URL courseUrl = CoachingLargeTest.class.getResource("CoachingCourse.zip");
		RepositoryEntry re = JunitTestHelper.deployCourse(null, "Coaching course", RepositoryEntryStatusEnum.published,
				courseUrl, defaultUnitTestOrganisation);
		dbInstance.commitAndCloseSession();
		...
		boolean canCoach = coachingDAO.isMember(owner, GroupRoles.owner, RepositoryEntryStatusEnum.preparationToClosed());
		Assert.assertTrue(canCoach);
	}
	
	@Test
	public void isMemberByCoach() {
		URL courseUrl = CoachingLargeTest.class.getResource("CoachingCourse.zip");
		RepositoryEntry re = JunitTestHelper.deployCourse(null, "Coaching course", RepositoryEntryStatusEnum.published,
				courseUrl, defaultUnitTestOrganisation);
		dbInstance.commitAndCloseSession();
		...
		boolean canCourseCoach = coachingDAO.isMember(courseCoach, GroupRoles.coach, RepositoryEntryStatusEnum.coachPublishedToClosed());
