 ...
 *
 */
public class UserHandlerTest extends OlatTestCase {
	
	@Autowired
	private TitleCreator titleCreator;
	
	@Test
	public void shouldMergeUserProperty() {
		Identity identity = JunitTestHelper.createAndPersistIdentityAsRndUser("");
		User user = identity.getUser();
		String value = random();
		user.setProperty(UserConstants.LASTNAME, value);
		String template = "$" + UserConstants.LASTNAME;
		
		String merged = titleCreator.merge(template, asList(user));
