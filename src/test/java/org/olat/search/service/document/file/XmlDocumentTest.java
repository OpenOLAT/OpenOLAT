...
public class XmlDocumentTest {
	...
	throws DocumentException, DocumentAccessException, URISyntaxException, IOException {
		URL url = XmlDocumentTest.class.getResource("test.xml");
		Assert.assertNotNull(url);

		VFSLeaf doc = new VFSJavaIOFile("test.xml", new File(url.toURI()));
		XmlDocument document = new XmlDocument();
		FileContent content =	document.readContent(doc);
		Assert.assertNotNull(content);
		String body = content.getContent();
		Assert.assertTrue(body.trim().contains("This is XML"));
	}
